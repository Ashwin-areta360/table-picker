...
def handle_query(payload: QueryRequest):
    ...
    of table names selected by the table-picker pipeline.
    """
    ...
    try:
        table_names: List[str] = search_service.get_final_tables(payload.query, role=payload.role)
    except Exception as exc:
        raise HTTPException(
            status_code=500,
    ...
    return QueryResponse(tables=table_names)
