...
class SchemaSelectorService:
    ...
    def select_final_tables(self, query: str, candidate_tables: List[TableMetadata]) -> List[str]:
        ...
        system_instruction = (
            ...
            "3. VALUE/FILTER MATCHING: When the user mentions values like 'completed', 'enrolled', 'active', "
            ...
            "4. HINT UTILIZATION: Prioritize columns marked as 'good_for_indexing' or 'good_for_filtering'.\n"
        ...
        TASK:
        - Match query terms to columns: synonyms, descriptions, AND sample values (e.g., 'completed' -> Status with values Completed/Enrolled).
    ...
    def _build_schema_context(self, tables: List[TableMetadata]) -> str:
        ...
        for table in tables:
            ...
            for col in table.columns.values():
                # Include synonyms, hints, and sample values to help LLM match query terms like 'completed', 'enrolled'
                syns = f" | Synonyms: {', '.join(col.synonyms)}" if col.synonyms else ""
                hints = f" | Hints: {', '.join(col.hints)}" if col.hints else ""
                samples = f" | Sample values: {', '.join(map(str, col.sample_values))}" if col.sample_values else ""
                col_details.append(f"  - {col.name}: {col.description}{syns}{hints}{samples}")
