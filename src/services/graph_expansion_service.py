...
class GraphExpansionService:
    ...
    def compute_join_result(self, selected_tables: List[str], candidate_tables: List[str]) -> Optional[JoinResult]:
        """
        Build the minimum join result for selected_tables using Kruskal's MST.
        ...
        # Kruskal's MST: sort by hop count, greedily add non-cycle edges
    ...
    def format_join_connectivity_text(self, join_paths: List[JoinPath]) -> str:
        """Format all-pairs join paths as a human-readable string for the LLM prompt."""
        if not join_paths:
            return ""
