...
class SchemaSelectorService:
    ...
    def select_final_tables(
        ...
        # Filter empty strings and unknown table names
        valid_names = {t.name for t in candidate_tables}
        selected = [s for s in selected if s and s in valid_names]

        # If the LLM returned nothing useful, pass all candidates to the SQL core
        if not selected:
            selected = [t.name for t in candidate_tables]
