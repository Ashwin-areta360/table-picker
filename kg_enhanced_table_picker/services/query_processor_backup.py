...
class QueryProcessor:
    ...
    SYNTACTIC_PATTERNS = [
        ...
        # JOIN patterns (more specific to avoid false positives)
        (r'(\w+)\s+(?:who|that|which)\s+(?:have|has|are|is)\s+', 'JOIN_WHO_PATTERN', 'JOIN'),
        (r'(\w+)\s+(?:enrolled|registered|assigned|associated)\s+(?:in|to|with)\s+', 'JOIN_ENROLLED_PATTERN', 'JOIN'),
    ...
    # NEW: Comparison operators (with more specific patterns to avoid false positives)
    COMPARISON_PATTERNS = {
        '>': ['greater than', 'more than', 'above', 'over', 'exceeds', 'higher than'],
        '>=': ['at least', 'minimum of', 'no less than', 'or more', 'or higher'],
        '<': ['less than', 'under', 'below', 'fewer than', 'lower than'],
        '<=': ['at most', 'maximum of', 'no more than', 'or less', 'or lower'],
        '=': ['equal to', 'equals', 'exactly'],  # Removed "is" - too generic
        '!=': ['not equal to', 'different from', 'other than'],  # More specific
        'BETWEEN': ['between', 'range'],  # Removed "from" - too generic
        'IN': ['one of'],  # Removed "in", "among", "within" - too generic
        'LIKE': ['similar to', 'containing', 'matching'],  # Removed "like" - too generic
    }
    ...
    # NEW: Join indicator words (more specific to avoid false positives)
    JOIN_INDICATOR_WORDS = {
        'who', 'that', 'which', 'whose',  # Relative pronouns (strong indicators)
        'enrolled', 'assigned', 'registered', 'associated',  # Relationship verbs
        # Removed generic prepositions: 'with', 'having', 'in', 'from', 'by', 'for', 'of', 'and'
    }
    ...
    def _classify_intent_enhanced(self, query: str, analysis: QueryAnalysis) -> QueryIntent:
        ...
        # Priority 1: RANKING (TOP N queries) - must come before aggregation
        if analysis.limit_n and analysis.sorting_required:
            return QueryIntent.RANKING
        
        # Priority 2: GROUPING (aggregation + group by)
        if analysis.aggregation_types and analysis.grouping_required:
            return QueryIntent.GROUPING
        
        # Priority 3: AGGREGATION (without grouping)
        if analysis.aggregation_types:
            return QueryIntent.AGGREGATION
        
        # Priority 4: NESTED_QUERY (subquery indicators)
        if any('SUBQUERY' in p[0] for p in analysis.matched_patterns):
            return QueryIntent.NESTED_QUERY
        
        # Priority 5: JOIN (but only with strong indicators)
        # Avoid false positives from "with" and "in"
        strong_join_indicators = {'who', 'that', 'which', 'enrolled', 'assigned', 'registered'}
        if any(ind in query_lower for ind in strong_join_indicators):
            if any('JOIN' in p[0] and 'WITH' not in p[0] for p in analysis.matched_patterns):
                return QueryIntent.JOIN
        
        # Priority 6: TEMPORAL (date/time filtering)
        if analysis.temporal_context:
            return QueryIntent.TEMPORAL
        
        # Priority 7: COMPARISON (but only with strong comparison operators)
        # Avoid false positives from "is" and "in"
        strong_comparisons = {'>', '>=', '<', '<=', '!=', 'BETWEEN'}
        if any(op in strong_comparisons for op in analysis.comparison_operators):
            return QueryIntent.COMPARISON
        
        # Priority 8: FILTERING (has "with" + conditions, but not join)
        if 'with' in query_lower and analysis.comparison_operators:
            return QueryIntent.FILTERING
        
        # Fallback to Phase 2 intent if spaCy available
        if self.backend == "spacy" and self.enable_phase2:
            spacy_intent = self.classify_intent(query)
            # Trust spaCy for LOOKUP, LISTING, UPDATE
            if spacy_intent in {QueryIntent.LOOKUP, QueryIntent.LISTING, QueryIntent.UPDATE}:
                return spacy_intent
